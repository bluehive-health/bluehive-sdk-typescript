...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { BLUEHIVE_API_KEY: requireValue(readEnv('BLUEHIVE_API_KEY') ?? client.apiKey, 'set BLUEHIVE_API_KEY environment variable or provide apiKey client option'), BLUE_HIVE_BASE_URL: readEnv('BLUE_HIVE_BASE_URL') ?? client.baseURL ?? undefined };
