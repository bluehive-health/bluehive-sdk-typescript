...
const prompt = `Runs JavaScript code to interact with the BlueHive API.
...
export function codeTool(params: { blockedMethods: SdkMethod[] | undefined }): McpTool {
  ...
  const handler = async (client: BlueHive, args: any): Promise<ToolCallResult> => {
    ...
    const res = await fetch(codeModeEndpoint, {
      ...
      headers: {
        ...
        client_envs: JSON.stringify({
          BLUEHIVE_API_KEY: requireValue(
            readEnv('BLUEHIVE_API_KEY') ?? client.apiKey,
            'set BLUEHIVE_API_KEY environment variable or provide apiKey client option',
          ),
          BLUE_HIVE_BASE_URL: readEnv('BLUE_HIVE_BASE_URL') ?? client.baseURL ?? undefined,
        }),
      },
      body: JSON.stringify({
        project_name: 'bluehive',
        code,
        intent,
        client_opts: {},
